# cl-nav-bar
## 说明
该组件是根据uni-nav-bar二次封装所得，组件属性与方法基本一致。可以实现自动从page.josn里面获取标题。
## 使用方式
	```html
	<template>
		<view class="content">
...
| clickLeft						| 导航栏左侧点击事件																																				| Function											| -							|
| clickRight						| 导航栏右侧点击事件																																						| Function											| -								|

# 事件列表
|事件名称					| 			说明		|     参数 |返回值 |
| -----------	| -----------			|   ----------- |     ----------- |     
|getHeight| 获取导航栏真实高度			|   	-		|  Number |    
|clickLeft| 左侧按钮点击时触发			|   	-		|  Number |    
|clickRight| 右侧按钮点击时触发			|   	-		|  Number |    
# 插槽列表
|插槽名称					| 			说明		|     参数 |
| -----------	| -----------			|   ----------- |         
|left| 向导航栏左侧插入			|   	-		|  
